		...
		if !info.IsDir() && strings.HasSuffix(info.Name(), ".txt") {
			...
			if len(newLinks) > 0 {
				for _, link := range newLinks {
					links = append(links, Link{path, link})
				}
			}
		}
...
func extractLink(line string) string {
	...
	if startIndex == -1 {
		startIndex = strings.Index(line, "https://")
	}
...
func isURLChar(char byte) bool {
	if char >= 'a' && char <= 'z' || char >= 'A' && char <= 'Z' || char >= '0' && char <= '9' || char == '.' || char == '-' || char == '_' || char == '~' || char == '/' || char == '?' || char == '=' || char == '&' || char == ':' || char == '#' {
		return true
	}
